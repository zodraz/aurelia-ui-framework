...
(function () {
	...
	AbstractChosen = (function () {
		...
		AbstractChosen.prototype.winnow_results = function () {
			...
			zregex = new RegExp(getAscii(escapedSearchText), 'i');
			regex = this.get_search_regex(escapedSearchText);
			eregex = new RegExp('^' + getAscii(escapedSearchText) + '$', 'i');
			_ref = this.results_data;
			for (_i = 0, _len = _ref.length; _i < _len; _i++) {
				option = _ref[_i];
				...
				if (this.include_option_in_results(option)) {
					...
					if (!(option.group && !this.group_search)) {
						...
						exact_result = exact_result || eregex.test(getAscii(option.html));
						if (option.search_match) {
							if (searchText.length) {
								startpos = getAscii(option.search_text).search(zregex);
		...
		AbstractChosen.prototype.get_search_regex = function (escaped_search_string) {
			...
			return new RegExp(regex_anchor + getAscii(escaped_search_string), 'i');
		};

		AbstractChosen.prototype.search_string_match = function (search_string, regex) {
			var part, parts, _i, _len;
			if (regex.test(getAscii(search_string))) {
				return true;
	...
	Chosen = (function (_super) {
		...
		Chosen.prototype.canCreate = false;
